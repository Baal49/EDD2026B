#ifndef COLAS_PRIORIDAD_H
#define COLAS_PRIORIDAD_H
...
public:
    ...
    T pop2(int& prioridadRecuperada) {
        if (cabeza == nullptr) {
            cout << "Cola de prioridad vacia" << endl;
            return T();
        }
        ...
        // Rescatamos ambos valores
        T valor = temp->getDato().dato;
        prioridadRecuperada = temp->getDato().prioridad;
...
public:
    ...
    T pop2(int& prioridadRecuperada) {
        for (int i = 0; i < maxPrioridades; i++) {
            if (tabla[i]->getSize() > 0) { 
                sizeTotal--;
                prioridadRecuperada = i; // <--- El índice es la prioridad
                return tabla[i]->pop();  
            }
        }
        cout << "Cola de prioridad vacia" << endl;
    ...
    void mostrar() const {
        for (int i = 0; i < maxPrioridades; i++) {
            cout << "Prioridad [" << i << "] (Elementos: " << tabla[i]->getSize() << ")\n";
        }
    }
